...
import okserver.download.db.DownloadRequest;
...
public class DownloadInfo implements Comparable<DownloadInfo> {
    ...
    public static ContentValues buildContentValues(DownloadInfo downloadInfo) {
        ...
        values.put(BOOKID, downloadInfo.getBookid());
    ...
    public static DownloadInfo parseCursorToBean(Cursor cursor) {
        ...
        info.setImg(cursor.getString(cursor.getColumnIndex(DownloadInfo.IMG)));
        info.setBookid(cursor.getString(cursor.getColumnIndex(DownloadInfo.BOOKID)));
