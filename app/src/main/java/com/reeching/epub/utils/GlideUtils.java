...
import static com.reeching.epub.activity.RegisterActivity.SD_HEAD;


/**
 * Created by 绍轩 on 2017/11/1.
 */
...
public class GlideUtils {
    ...
    //图片保存到本地
    public static void loadImageViewSD(Context mContext, String path) {
        Glide.with(mContext).load(path).asBitmap().toBytes().into(new SimpleTarget<byte[]>() {
            @Override
    ...
    private static void savaBitmap( byte[] bytes) {
        FileOutputStream fos = null;
        try {
            //保存路径
            File imgDir = new File(SD_HEAD);
            if (!imgDir.exists()) {
                imgDir.mkdirs();
            }
    ...
    //设置缓存策略
    public static void loadImageViewDiskCache(Context mContext, String path, ImageView mImageView,int errorImageView) {
        Glide.with(mContext).load(path).diskCacheStrategy(DiskCacheStrategy.ALL).error(errorImageView).into(mImageView);
    }
