...
//        getListView().setOnCreateContextMenuListener(this);
    ...
    protected void onListItemClick(ListView listView, View view, int position, long rowId) {
        ...
        LogUtils.i(tree.getName());
        final Book book = tree.getBook();
        if (book != null) {
            showBookInfo(book);
            LogUtils.i(tree.getBook().getPath());
            LogUtils.i(tree.getBook().getTitle());
            LogUtils.i(tree.getBook().getMyCoverPath());
        } else {
            LogUtils.i(tree.getName());
            openTree(tree);
        }
    ...
    private void showBookInfo(Book book) { //y 直接打开书籍，不再打开书籍详情 ---> BookInfoActivity
        KooReader.openBookActivity(LibraryActivity.this, book, null,null);
